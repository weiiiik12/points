...
function switchChild(idx) {
    ...
    // 💡 動作二完美對接：把舊的常態答題拿掉，換成最新的防刷限制 5 題小學堂渲染系統！
...
window.restoreDefaultPrizes = function() { if(masterData.settings.prizeScope==='individual') data.tiers=JSON.parse(JSON.stringify(DEFAULT_TIERS)); else masterData.tiers=JSON.parse(JSON.stringify(DEFAULT_TIERS)); saveData(); renderPrizeManager();
